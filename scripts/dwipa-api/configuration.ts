...
namespace NusaConfiguration {
    ...
    export function setConfigUI(player: Player, path: string = "") {
        const array = getArray(getConfig(), path);

        if (typeof array === "object" || typeof array === "undefined") {
            if (!PlayerRank.isAdmin(player)) return;
            ...
            for (const key of keys) {
                ...
                if (_newpath.startsWith(".")) _newpath=_newpath.substring(1);
                if (_newpath.endsWith(".")) _newpath=_newpath.substring(0, _newpath.length-2);
            ...
            if (!PlayerRank.hasPermission(player, "admin")) return;
            const form = new CustomForm("config.form-ui.setconfig.title", [
