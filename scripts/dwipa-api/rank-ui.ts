...
namespace RankPermsUI {
    ...
    export function main(player: Player, callback?: () => void) {
        ...
        form.sendTo(player, CommandPermissionLevel.ADMIN).then((res) => {
            ...
            if (res.selection === ranks.length+1) return callback ? callback() : null;
            if (res.selection === ranks.length) return createRank(player, (r) => r ? main(player) : true);
    ...
    export function deleteRankConfirm(player: Player, rankId: string, callback?: (response: boolean) => boolean|void) {
        const rank = RankPerms.getRank(rankId);
        if (!rank) return Translate.sendTranslate(player, "rankperms.error.notfound.rank");
    ...
    export function editRank(player: Player, rankId: string, callback?: () => void) {
        ...
        form.sendTo(player, CommandPermissionLevel.ADMIN).then((res) => {
            if (res.selection === undefined) return;
            if (res.selection === 10 && callback) return callback();

            if (res.selection === 0) return renameRank(player, rankId, (r, rankId) => r ? editRank(player, rankId, callback) : true);
            if (res.selection === 1) return rank.setDefault().then(() => editRank(player, rankId, callback)).catch((err) => typeof err === "string" ? Translate.sendTranslate(player, err) : null);
            if (res.selection === 2) return rank.moveRank("up").then(() => editRank(player, rankId, callback)).catch((err) => typeof err === "string" ? Translate.sendTranslate(player, err) : null);
            if (res.selection === 3) return rank.moveRank("down").then(() => editRank(player, rankId, callback)).catch((err) => typeof err === "string" ? Translate.sendTranslate(player, err) : null);
            if (res.selection === 4) return setDisplay(player, rankId, (r) => r ? editRank(player, rankId, callback) : true);
            if (res.selection === 5) return editPermissions(player, rankId, () => editRank(player, rankId, callback));
            if (res.selection === 6) return addInheritance(player, rankId, (r) => r ? editRank(player, rankId, callback) : true);
            if (res.selection === 7) return setFormatChat(player, rankId, (r) => r ? editRank(player, rankId, callback) : true);
            if (res.selection === 8) return setFormatName(player, rankId, (r) => r ? editRank(player, rankId, callback) : true);
            if (res.selection === 9) return deleteRankConfirm(player, rankId, (r) => r ? callback ? callback() : true : true);
        });
    }

    export function renameRank(player: Player, rankId: string, callback?: (response: boolean, rankId: string) => boolean|void) {
        const rank = RankPerms.getRank(rankId);
        if (!rank) return Translate.sendTranslate(player, "rankperms.error.notfound.rank");
        ...
        form.sendTo(player, CommandPermissionLevel.ADMIN).then((res) => {
            ...
            rank.renameRank(res.formValues[0] as string)
    ...
    export function setDisplay(player: Player, rankId: string, callback?: (response: boolean) => boolean|void) {
        ...
        form.sendTo(player, CommandPermissionLevel.ADMIN).then((res) => {
            ...
            rank.setDisplay(res.formValues[0] as string)
            .then(() => callback ? callback(true) : Translate.sendTranslate(player, "rankperms.success.setdisplay"))
            .catch((err) => typeof err === "string" ? callback && callback(false) ? Translate.sendTranslate(player, err) : null : null);
        });
    }

    export function setFormatChat(player: Player, rankId: string, callback?: (response: boolean) => boolean|void) {
        const rank = RankPerms.getRank(rankId);
        if (!rank) return Translate.sendTranslate(player, "rankperms.error.notfound.rank");
        ...
        form.sendTo(player, CommandPermissionLevel.ADMIN).then((res) => {
            ...
            rank.setFormatChat(res.formValues[0] as string)
            .then(() => callback ? callback(true) : Translate.sendTranslate(player, "rankperms.success.setformat"))
            .catch((err) => typeof err === "string" ? callback && callback(false) ? Translate.sendTranslate(player, err) : null : null);
        });
    }

    export function setFormatName(player: Player, rankId: string, callback?: (response: boolean) => boolean|void) {
        const rank = RankPerms.getRank(rankId);
        if (!rank) return Translate.sendTranslate(player, "rankperms.error.notfound.rank");
        ...
        form.sendTo(player, CommandPermissionLevel.ADMIN).then((res) => {
            ...
            rank.setFormatNameTag(res.formValues[0] as string)
            .then(() => callback ? callback(true) : Translate.sendTranslate(player, "rankperms.success.setformat"))
            .catch((err) => typeof err === "string" ? callback && callback(false) ? Translate.sendTranslate(player, err) : null : null);
        });
    }

    export function addInheritance(player: Player, rankId: string, callback?: (response: boolean) => boolean|void) {
        const rank = RankPerms.getRank(rankId);
        if (!rank) return Translate.sendTranslate(player, "rankperms.error.notfound.rank");
        ...
        form.sendTo(player, CommandPermissionLevel.ADMIN).then((res) => {
            if (res.selection === undefined) return;
            if (res.selection === ranks.length) return callback ? callback(false) : null;
    ...
    export function editPermissions(player: Player, rankId: string, callback?: () => void) {
        const rank = RankPerms.getRank(rankId);
        if (!rank) return Translate.sendTranslate(player, "rankperms.error.notfound.rank");
        ...
        form.sendTo(player, CommandPermissionLevel.ADMIN).then((res) => {
            if (res.selection === undefined) return;
            if (res.selection === permissions.length+1) return callback ? callback() : null;
            if (res.selection === permissions.length) return addPermission(player, rankId, (r) => r ? editPermissions(player, rankId, callback) : true);

            editPermission(player, rankId, permissions[res.selection], () => editPermissions(player, rankId, callback));
        });
    }

    export function editPermission(player: Player, rankId: string, permission: string, callback?: () => void) {
        const rank = RankPerms.getRank(rankId);
        if (!rank) return Translate.sendTranslate(player, "rankperms.error.notfound.rank");
        ...
        form.sendTo(player, CommandPermissionLevel.ADMIN).then((res) => {
            ...
            if (res.selection === 0) return setPermission(player, rankId, permission, (r) => r ? editPermission(player, rankId, permission, callback) : true);
            if (res.selection === 1) return removePermissionConfirm(player, rankId, permission, callback);
        });
    }

    export function setPermission(player: Player, rankId: string, permission: string, callback?: (response: boolean) => boolean|void) {
        const rank = RankPerms.getRank(rankId);
        if (!rank) return Translate.sendTranslate(player, "rankperms.error.notfound.rank");
        if (!rank.hasPermission(permission)) {
        ...
        form.sendTo(player, CommandPermissionLevel.ADMIN).then((res) => {
            ...
            rank.setPermission(permission, res.formValues[0] as string)
            .then(() => callback ? callback(true) : Translate.sendTranslate(player, "rankperms.success.setpermission"))
            .catch((err) => typeof err === "string" ? callback && callback(false) ? Translate.sendTranslate(player, err) : null : null);
        });
    }

    export function addPermission(player: Player, rankId: string, callback?: (response: boolean) => boolean|void) {
        const rank = RankPerms.getRank(rankId);
        if (!rank) return Translate.sendTranslate(player, "rankperms.error.notfound.rank");
        ...
        form.sendTo(player, CommandPermissionLevel.ADMIN).then((res) => {
            ...
            rank.addPermission(res.formValues[0] as string)
            .then(() => callback ? callback(true) : Translate.sendTranslate(player, "rankperms.success.addpermission"))
            .catch((err) => typeof err === "string" ? callback && callback(false) ? Translate.sendTranslate(player, err) : null : null);
        });
    }

    export function removePermissionConfirm(player: Player, rankId: string, permission: string, callback?: (response: boolean) => boolean|void) {
        const rank = RankPerms.getRank(rankId);
        if (!rank) return Translate.sendTranslate(player, "rankperms.error.notfound.rank");
        if (!rank.hasPermission(permission)) {
            callback ? callback(false) : null;
