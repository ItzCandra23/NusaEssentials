...
type RankAdminActionType = typeof RankAdminAction[number];

command.register("rankadm", "commands.rankadm.description", "RankPerms", CommandPermissionLevel.ADMIN, (p, player) => {
    const action = p.action as RankAdminActionType;
    if (!action) return RankPermsUI.main(player);

    if (action === "createrank") {
        if (!p.value1) return RankPermsUI.createRank(player);
    ...
    if (action === "deleterank") {
        if (!p.value1) return Translate.sendTranslate(player, "rankperms.error.notfound.rank");
    ...
    if (action === "renamerank") {
        if (!p.value1) return Translate.sendTranslate(player, "rankperms.error.notfound.rank");
        if (!p.value2) return RankPermsUI.renameRank(player, p.value1);
    ...
    if (action === "setdisplay") {
        if (!p.value1) return Translate.sendTranslate(player, "rankperms.error.notfound.rank");
        if (!p.value2) return RankPermsUI.setDisplay(player, p.value1);
    ...
    if (action === "editchat") {
        if (!p.value1) return Translate.sendTranslate(player, "rankperms.error.notfound.rank");
        if (!p.value2) return RankPermsUI.setFormatChat(player, p.value1);
    ...
    if (action === "editnametag") {
        if (!p.value1) return Translate.sendTranslate(player, "rankperms.error.notfound.rank");
        if (!p.value2) return RankPermsUI.setFormatName(player, p.value1);
    ...
    if (action === "listpermissions") {
        const rank = RankPerms.getRank(p.value1 ?? "");
    ...
    if (action === "addperm") {
        if (!p.value1) return Translate.sendTranslate(player, "rankperms.error.notfound.rank");
        if (!p.value2) return RankPermsUI.addPermission(player, p.value1);
    ...
    if (action === "delperm") {
        ...
        RankPerms.removePermission(p.value1, p.value2 ?? "")
    ...
    if (action === "addinheritance") {
        if (!p.value1) return Translate.sendTranslate(player, "rankperms.error.notfound.rank");
        if (!p.value2) return RankPermsUI.addInheritance(player, p.value1);
    ...
    if (action === "setrank") {
        const playerId = PlayerData.getPlayerId(p.value1 ?? "");
        if (!playerId) return Translate.sendTranslate(player, "playerrank.error.notfound.player");
        if (!p.value2) return Translate.sendTranslate(player, "playerrank.error.notfound.rank");
