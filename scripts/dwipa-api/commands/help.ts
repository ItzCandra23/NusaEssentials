import { command, CommandParameters, CommandPermissionLevel, CustomCommandFactory } from "../command";
...
command.register("help", "commands.help.description", "Commands", CommandPermissionLevel.NORMAL, (p, player) => {
    if (!p.page || Number(p.page) === Number(p.page)) {
        ...
        if (!cmd || !CustomCommandFactory.hasPermission(player, cmd.permission)) return Translate.sendTranslate(player, "commands.help.error");
